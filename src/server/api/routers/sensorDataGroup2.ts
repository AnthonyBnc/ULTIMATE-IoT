...
export const sensorDataGroup2Router = createTRPCRouter({
    getAll: publicProcedure.query(async ({ctx}) => {
        const data = await ctx.prisma.sensorDataGroup2.findMany({
            orderBy: {
                timestamp: 'desc'
            }
        });
    ...
    countAlert: publicProcedure.query(async ({ ctx }) => {
        ...
        const countAlert = await ctx.prisma.sensorDataGroup2.count({
          where: {
            timestamp: {
              gte: oneHourAgo,
            },
      ...
      getChangesFromSecondLatest: publicProcedure.query(async ({ ctx }) => {
        ...
        return {
          ...
          light_level: calculateChange(latest.light_level, secondLatest.light_level)?.toFixed(2),
        };
      }),
